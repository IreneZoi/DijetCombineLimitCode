...
outdir = "/nfs/dust/cms/user/zoiirene/DiBoson/MyPlots/Ftest/"
mjjcut = "1055"
sqrtS = 13000.
lumi = 36814.
 ...
 #  //Draw the graph, the function and the confidence intervals
  ...
  for bin in range (1,hMassNEW.GetNbinsX()):
    if( hMassNEW.GetXaxis().GetBinLowEdge(bin)>=fFitXmin and hMassNEW.GetXaxis().GetBinUpEdge(bin)<=fFitXmax ):
       ...
       fit = BKGfit.Integral(hMassNEW.GetXaxis().GetBinLowEdge(bin) , hMassNEW.GetXaxis().GetBinUpEdge(bin) ) #irene
...
#  addInfo.AddText(fLabel)
  addInfo.AddText("VV Signal Region")
  addInfo.AddText("|#eta| < 2.5, p_{T} > 200 GeV")
  addInfo.AddText("0 < #tau_{21} < 0.35")
...
#      performFit(file1,"DijetMassLowPuri%s"%ch, len(massBins)-1, massBins, 1118, fitmax, "%s category, LP"%ch,"%s%sLP"%(outdir,ch),"summer16" ,doSigmaBand=False)
      performFit(file1,"qcd%s"%ch, len(massBins)-1, massBins, 1118, fitmax, "qcd %s category, SR"%ch,"%s%s_qcdSR"%(outdir,ch),"summer16" ,doSigmaBand=False)
      performFit(file1,"data%s"%ch, len(massBins)-1, massBins, 1118, fitmax, "data %s category, SR"%ch,"%s%s_dataSR"%(outdir,ch),"summer16" ,doSigmaBand=False)
