...
void PrepareInvertedRootFile(TString cut, TString inverted, TString legend = "yes", TString status = "blinded"){
  ...
  */
  ...
  d_WTopJet_withVBF_VV_data =(TDirectoryFile*)f__inverted_data->Get(selectionVBF);
  ...
  for(j=0;j<Channels;j++)
      ...
      for(i=0;i<MassPoints;i++)
	{
	  ...
	  h_WTopJet_invM_withVBF_VV_qcd_inverted->Write("data_invMass");
	  h_WTopJet_invM_withVBF_VV_qcd_inverted->Write("qcd_invMass");
