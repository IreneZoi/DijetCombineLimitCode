...
void PrepareRootAllFile(TString cut, TString directory, TString legend = "yes", TString status = "blinded"){
  cout << " cut " << cut << endl;
  // input 
  ...
  for(j=0;j<Channels;j++)
    {
      for(i=0;i<MassPoints;i++)
   	{
   	  signal_file = "uhh2.AnalysisModuleRunner.MC.VBF_"+Channel[j]+ss_mass[i]+"To"+Decay[j]+"_inv_newJEC_PUPPI.root";
   	  if(print) std::cout << "signal_file : " << signal_file  <<std::endl;
   	  signal_file = base_dir+signal_file;
   	  f_signal[j][i] = new TFile(signal_file); 
  ...
  for(i=0; i<Channels; i++)
    {
      for(j=0; j<MassPoints;j++)
	{ 
	  f_signal[i][j]->cd();
      
	  d_WTopJet[i][j] =(TDirectoryFile*)f_signal[i][j]->Get(selectionVV);
	  h_WTopJet_invM_VV[i][j] = (TH1F*)d_WTopJet[i][j]->Get("invMass_rootfile");                                                                                    
	  if(print) std::cout << " histo VV " <<std::endl;
	  d_WTopJet_withVBF_VV[i][j] =(TDirectoryFile*)f_signal[i][j]->Get(selectionVBF);
	  h_WTopJet_invM_withVBF_VV[i][j] = (TH1F*)d_WTopJet_withVBF_VV[i][j]->Get("invMass_rootfile");                       
  ...
  for(j=0;j<Channels;j++)
    {
      for(i=0;i<MassPoints;i++)
	{
	  output[j][i] = new TFile(output_dir+channel[j]+"_"+cut+"_"+ss_mass[i]+".root","RECREATE");
	  h_WTopJet_invM_VV[j][i]->Write(channel[j]+"_invMass");
	  h_WTopJet_invM_VV_data->Write("data_invMass");
	  h_WTopJet_invM_VV_qcd->Write("qcd_invMass");
	  
	  h_WTopJet_invM_withVBF_VV[j][i]->Write(channel[j]+"_invMass_afterVBFsel");
