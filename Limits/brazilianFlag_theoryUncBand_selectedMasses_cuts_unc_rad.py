...
#        gr1up.SetPoint(j, radmasses[j], y1up[j])
 #       grmean.SetPoint(j, radmasses[j], ymean[j])
  #      gr1down.SetPoint(j, radmasses[j], y1down[j])    
    ...
    text_file = open("LimitTxt/"+label+"_Limit_Unc.txt", "w")
    text_file.write("1200 2000 4000\n")    
    text_file.write("{0} {1} {2} #limit\n".format( str(grmean.Eval(1.2)), str(grmean.Eval(2.0)), str(grmean.Eval(4.0)) )    )
    text_file.write("{0} {1} {2} #+1sigma\n".format( str(gr1up.Eval(1.2)), str(gr1up.Eval(2.0)), str(gr1up.Eval(4.0)) )    )
    text_file.write("{0} {1} {2} #-1sigma\n".format( str(gr1down.Eval(1.2)), str(gr1down.Eval(2.0)), str(gr1down.Eval(4.0)) )    )
    text_file.close()
    #===============================================================================================
...
#          masses =[2000]
          ...
          for mass in masses:
              combinedplots+=[postfix+"CMS_jj_"+str(mass)+"_"+chan+"_"+str(cut)+"_13TeV_"+region+"_"+str(unc)+"_asymptoticCLs_new.root"]
          print combinedplots;
          if region == "_invMass":
              Plot(combinedplots,chan+"_"+region+"_"+str(cut)+"_"+str(unc)+"_new_combined", obs=True,CompareLimits=False,plotExpLimitRatio="")  
          if region == "_invMass_afterVBFsel":
              Plot(combinedplots,chan+"_"+region+"_"+str(cut)+"_"+str(unc)+"_new_combined", obs=False,CompareLimits=False,plotExpLimitRatio="")  
