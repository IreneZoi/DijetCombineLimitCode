...
def Plot(files_failVBF,files_passVBF,files_combine, label, obs,CompareLimits=False,plotExpLimitRatio=""):
    
    radmasses = []
    for f in files_combine:
      if not postfix:
    ...
    for onefile in files_failVBF:
        ...
        limits_failVBF = []
        for quantile in tree:
            limits_failVBF.append(tree.limit)
            print ">>>   %.2f" % limits_failVBF[-1]
        rad_failVBF.append(limits_failVBF[:6])
    for onefile in files_passVBF:
        print "using file " + onefile
        file = rt.TFile(onefile)
        tree = file.Get("limit")
        print tree
        limits_passVBF = []
        for quantile in tree:
            limits_passVBF.append(tree.limit)
            print ">>>   %.2f" % limits_passVBF[-1]
        rad_passVBF.append(limits_passVBF[:6])
    for onefile in files_combine:
        print "using file " + onefile
        file = rt.TFile(onefile)
        tree = file.Get("limit")
        print tree
        limits_combine = []
        for quantile in tree:
            limits_combine.append(tree.limit)
            print ">>>   %.2f" % limits_combine[-1]
        rad_combine.append(limits_combine[:6])


    print limits_failVBF
    print limits_passVBF
    print limits_combine
    ...
    for i in range(0,len(efficiencies)):
        ...
        print "rad FAIL i 2 " + str(rad_failVBF[i][2])        
        print "rad PASS i 2 " + str(rad_passVBF[i][2])        
        print "rad COMBINE i 2 " + str(rad_combine[i][2])        
        print "radmasses i " + str(radmasses[i])        
        print "efficiency  " + str(efficiencies[radmasses[i]])        

        y2up_f.append(rad_failVBF[i][0]*efficiencies[radmasses[i]])
        y1up_f.append(rad_failVBF[i][1]*efficiencies[radmasses[i]])
        ymean_f.append(rad_failVBF[i][2]*efficiencies[radmasses[i]])
        y1down_f.append(rad_failVBF[i][3]*efficiencies[radmasses[i]])
        y2down_f.append(rad_failVBF[i][4]*efficiencies[radmasses[i]])
        yobs_f.append(rad_failVBF[i][5]*efficiencies[radmasses[i]])

        y2up_p.append(rad_passVBF[i][0]*efficiencies[radmasses[i]])
        y1up_p.append(rad_passVBF[i][1]*efficiencies[radmasses[i]])
        ymean_p.append(rad_passVBF[i][2]*efficiencies[radmasses[i]])
        y1down_p.append(rad_passVBF[i][3]*efficiencies[radmasses[i]])
        y2down_p.append(rad_passVBF[i][4]*efficiencies[radmasses[i]])
        yobs_p.append(rad_passVBF[i][5]*efficiencies[radmasses[i]])

        y2up_c.append(rad_combine[i][0]*efficiencies[radmasses[i]])
        y1up_c.append(rad_combine[i][1]*efficiencies[radmasses[i]])
        ymean_c.append(rad_combine[i][2]*efficiencies[radmasses[i]])
        y1down_c.append(rad_combine[i][3]*efficiencies[radmasses[i]])
        y2down_c.append(rad_combine[i][4]*efficiencies[radmasses[i]])
        yobs_c.append(rad_combine[i][5]*efficiencies[radmasses[i]])
    ...
    grmean_fail = rt.TGraphErrors(1)
    grmean_fail.SetLineColor(rt.kBlue)
    grmean_fail.SetLineWidth(4)
    grmean_fail.SetLineStyle(3) #irene
    grmean_fail.SetMarkerStyle(20) #irene
    grmean_fail.SetMarkerColor(rt.kBlue) #irene

    grmean_pass = rt.TGraphErrors(1)
    grmean_pass.SetLineColor(rt.kRed)
    grmean_pass.SetLineWidth(4)
    grmean_pass.SetLineStyle(1) #irene
    grmean_pass.SetMarkerStyle(20) #irene
    grmean_pass.SetMarkerColor(rt.kRed) #irene

  
    for j in range(0,len(radmasses)):
        grobs.SetPoint(j, radmasses[j], yobs_c[j])
        gr2up.SetPoint(j, radmasses[j], y2up_c[j])
        gr1up.SetPoint(j, radmasses[j], y1up_c[j])
        grmean.SetPoint(j, radmasses[j], ymean_c[j])
        grmean_fail.SetPoint(j, radmasses[j], ymean_f[j])
        grmean_pass.SetPoint(j, radmasses[j], ymean_p[j])
        gr1down.SetPoint(j, radmasses[j], y1down_c[j])    
        gr2down.SetPoint(j, radmasses[j], y2down_c[j])
       
    
    mg.Add(gr2up)#.Draw("same")
    mg.Add(gr1up)#.Draw("same")
    mg.Add(grmean_fail,"L")#.Draw("same,AC*")
    mg.Add(grmean_pass,"L")#.Draw("same,AC*")
    mg.Add(grmean,"L")#.Draw("same,AC*")
    mg.Add(gr1down)#.Draw("same,AC*")
    mg.Add(gr2down)#.Draw("same,AC*")
    if obs: mg.Add(grobs,"L")#.Draw("AC*")
    ...
    frame = c1.DrawFrame(1.1,0.001, 4.6, 10)
...
#    frame.GetYaxis().SetTitle("#sigma_{95%} #times BR("+resonance+" #rightarrow WW"+label.split("_")[0].replace("graviton","").replace("radion","")+") (pb)") #irene 
    ...
    for i in range(0,n):
        grgreen.SetPoint(i,radmasses[i],y2up_c[i])
        grgreen.SetPoint(n+i,radmasses[n-i-1],y2down_c[n-i-1])
    ...
    for i in range(0,n):
        gryellow.SetPoint(i,radmasses[i],y1up_c[i])
        gryellow.SetPoint(n+i,radmasses[n-i-1],y1down_c[n-i-1])
    ...
    leg3 = rt.TLegend(0.4,0.5,0.9446734,0.600000)
    ...
    leg2.AddEntry(grmean, " ", "L")
    leg3.AddEntry(grmean_fail, " fail VBF", "L")
    leg3.AddEntry(grmean_pass, " pass VBF", "L")
...
#          masses =[2000]
          ...
          combinedplots_failVBF=[]
          combinedplots_passVBF=[]
          combinedplots_combine=[]
    
          for mass in masses:
              combinedplots_failVBF +=[postfix+"CMS_jj_"+str(mass)+"_"+chan+"_"+str(cut)+"_13TeV_"+regions[0]+"_asymptoticCLs_new.root"]
              combinedplots_passVBF +=[postfix+"CMS_jj_"+str(mass)+"_"+chan+"_"+str(cut)+"_13TeV_"+regions[1]+"_asymptoticCLs_new.root"]
              combinedplots_combine +=[postfix+"CMS_jj_"+str(mass)+"_"+chan+"_"+str(cut)+"_13TeV_"+regions[2]+"_asymptoticCLs_new.root"]

          print combinedplots_failVBF;
          print combinedplots_passVBF;
          print combinedplots_combine;
#          if region == "_invMass":
          Plot(combinedplots_failVBF,combinedplots_passVBF,combinedplots_combine,chan+"_"+str(cut)+"_allCat_combined", obs=False,CompareLimits=False,plotExpLimitRatio="")  
