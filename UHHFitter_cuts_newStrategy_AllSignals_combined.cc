...
void runfits(const string cuts="none",const Float_t mass=2000, int signalsample = 1, int channel = 1,std::string altfunc="", Bool_t dobands = false)
{
  ...
  if (signalsample==2){
    signalname="wprime_"+cuts;
  }
  if (signalsample==3){
    signalname="zprime_"+cuts;
  }
...
void AddSigData(RooWorkspace* w, Float_t mass, int signalsample, std::vector<string> cat_names, string cut) {
  ...
  if (signalsample==2) {
    sigfilename = TString(Form("dijetUHH_13TeV_wprime_%s_Interpolated%d_miniTree.root", cut.c_str(), iMass));
  }
  if (signalsample==3) {
    sigfilename = TString(Form("dijetUHH_13TeV_zprime_%s_Interpolated%d_miniTree.root", cut.c_str(), iMass));
  }
...
void MakeDataCard_1Channel(std::string altfunc, RooWorkspace* w, const char* fileBaseName, const char* fileBkgName, int iChan, TString signalname, int signalsample, std::vector<string> cat_names, double mass) {
  ...
  if(signalsample==0){
    outFile << Form("shapes graviton_jj %s ", cat_names[iChan].c_str()) << wsDir+TString(fileBaseName)+".root" << Form(" w_all:"+signalname+"_jj_sig_%s", cat_names[iChan].c_str()) << endl;
    ...
    outFile << Form("shapes radion_jj %s ", cat_names[iChan].c_str()) << wsDir+TString(fileBaseName)+".root" << Form(" w_all:"+signalname+"_jj_sig_%s", cat_names[iChan].c_str()) << endl;
  ...
  else if(signalsample==2){
    outFile << Form("shapes wprime_jj %s ", cat_names[iChan].c_str()) << wsDir+TString(fileBaseName)+".root" << Form(" w_all:"+signalname+"_jj_sig_%s", cat_names[iChan].c_str()) << endl;
    //    outFile << Form("shapes radion_jj %s ", cat_names[iChan].c_str()) << wsDir+TString::Format(("CMS_jj_"+altfunc+"radion_%.0f_13TeV.root").c_str(), mass ) << Form(" w_all:radion_jj_sig_%s", cat_names[iChan].c_str()) << endl;
  } 
  else if(signalsample==3){
    outFile << Form("shapes zprime_jj %s ", cat_names[iChan].c_str()) << wsDir+TString(fileBaseName)+".root" << Form(" w_all:"+signalname+"_jj_sig_%s", cat_names[iChan].c_str()) << endl;
    //    outFile << Form("shapes radion_jj %s ", cat_names[iChan].c_str()) << wsDir+TString::Format(("CMS_jj_"+altfunc+"radion_%.0f_13TeV.root").c_str(), mass ) << Form(" w_all:radion_jj_sig_%s", cat_names[iChan].c_str()) << endl;
  } 
  ...
  if(signalsample==0){
    ...
    outFile << "bin                     "<< Form("%s       %s      ", cat_names[iChan].c_str(), cat_names[iChan].c_str() )<< endl;
    outFile << "process                 wprime_jj     bkg_fit_jj     " << endl;
    outFile << "process                 0               1          " << endl;
    outFile << "rate                    "<< signal[iChan]->sumEntries()*scaleFactor<< "         " << 1 << endl;
    ...
    outFile << "bin                     "<< Form("%s       %s      ", cat_names[iChan].c_str(), cat_names[iChan].c_str() )<< endl;
    outFile << "process                 zprime_jj     bkg_fit_jj     " << endl;
    outFile << "process                 0               1          " << endl;
    outFile << "rate                    "<< signal[iChan]->sumEntries()*scaleFactor<< "         " << 1 << endl;
...
void UHHFitter_cuts_newStrategy_AllSignals_combined(string cut, double mass, int signalsamples=0, int channel=0,std::string altfunc="",std::string postfix="")
{
  filePOSTfix=postfix;
